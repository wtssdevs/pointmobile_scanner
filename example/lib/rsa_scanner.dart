...
import 'package:pointmobile_scanner_example/zar_id_doc.dart';
...
class _RsaScannerState extends State<RsaScanner> {
  ...
  Future _startLiveFeed() async {
    final camera = await availableCameras().then((cameras) => cameras.firstWhere((c) => c.lensDirection == CameraLensDirection.back));
    _controller = CameraController(camera, ResolutionPreset.veryHigh, enableAudio: false);
  ...
  InputImage _inputImageFromCameraImage(CameraImage image) {
    ...
    final inputImageFormat = InputImageFormatValue.fromRawValue(image.format.raw ?? 17);
