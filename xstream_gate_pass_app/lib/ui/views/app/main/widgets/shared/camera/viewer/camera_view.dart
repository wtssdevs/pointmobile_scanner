...
import 'package:xstream_gate_pass_app/ui/views/app/main/widgets/shared/camera/viewer/camera_view_model.dart';
...
class CameraView extends StatefulWidget {
  ...
  const CameraView({Key? key, required this.refId, required this.referanceId, required this.fileStoreType}) : super(key: key);

  @override
  State<CameraView> createState() => _CameraCaptureViewState();
}
...
class _CameraCaptureViewState extends State<CameraView> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (_mediaService.cameras.isNotEmpty) {
      onNewCameraSelected(_mediaService.cameras.firstWhere((e) => e.lensDirection == CameraLensDirection.back));
    }
  }
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<CameraViewModel>.reactive(
      viewModelBuilder: () => CameraViewModel(widget.refId, widget.referanceId, widget.fileStoreType, this),
        ...
        body: SafeArea(
          child: Column(
            children: <Widget>[
              ...
              //_captureControlRowWidget(model),
                    ...
                    CameraButton(
                      ...
                      onTap: () {
                        if (controller != null && controller!.value.isInitialized && !controller!.value.isRecordingVideo) {
                          onTakePictureButtonPressed(model);
                        }
                      },
                    ),
                    const SizedBox(
  ...
  Widget _captureControlRowWidget(CameraViewModel model) {
    final CameraController? cameraController = controller;
    ...
    return Row(
      ...
      children: <Widget>[
        ...
        //   color: Colors.blue,
        //   onPressed: cameraController != null && cameraController.value.isInitialized && !cameraController.value.isRecordingVideo ? onTakePictureButtonPressed(model) : null,
        // ),
        CameraButton(
          key: const ValueKey('cameraButton'),
          captureMode: null,
          isRecording: false,
          onTap: () {
            //  cameraController != null && cameraController.value.isInitialized && !cameraController.value.isRecordingVideo ? onTakePictureButtonPressed(model) : null;
  ...
  Widget _cameraTogglesRowWidget() {
    ...
    if (_mediaService.cameras.isEmpty) {
      SchedulerBinding.instance.addPostFrameCallback((_) async {
        showInSnackBar('No camera found.');
      });
      return const Text('None');
    } else {
      for (final CameraDescription cameraDescription in _mediaService.cameras) {
        toggles.add(
          SizedBox(
            width: 90.0,
  ...
  onTakePictureButtonPressed(CameraViewModel model) async {
